/****** Object:  StoredProcedure [dbo].[ClientMIFieldSet_Fetch]    Script Date: 05/08/2012 16:20:58 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	--IF @pCaseContactID > 0
	--BEGIN
	--	EXEC [ClaimantMIFieldSet_Fetch]	@pCaseID = @pCase_CaseID, @pContactID = @pCaseContactID	
	--	RETURN 
	--END
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT mf.MIFieldDefinition_MIFieldCode, mf.MIFieldDefinition_DataField, mf.MIFieldDefinition_MIFieldTypeCode 
				FROM [Case] c INNER JOIN
				ClientMIDefinition cm ON c.Case_ClientCode = cm.ClientMIDefinition_MIDEFCODE and cm.ClientMIDefinition_Inactive = 0 INNER JOIN
				ClientMIFieldSet cf ON cm.ClientMIDefinition_MIDEFCODE = cf.ClientMIFieldSet_ClientMIDEFCODE and cf.ClientMIFieldSet_Inactive = 0 INNER JOIN
				MIFieldDefinition mf ON cf.ClientMIFieldSet_MIFieldDefCode = mf.MIFieldDefinition_MIFieldCode and mf.MIFieldDefinition_Inactive = 0
				WHERE c.Case_CaseID = @pCase_CaseID and mf.MIFieldDefinition_DataField <> 'n/a' and ISNULL(mf.MIFieldDefinition_DataField,'') <> ''	
			END
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						--and mf.MIFieldDefinition_DataField <> 'n/a' 
						--and ISNULL(mf.MIFieldDefinition_DataField,'') <> ''
			...
			BEGIN
				...
				SELECT mf.MIFieldDefinition_MIFieldCode, mf.MIFieldDefinition_DataField, mf.MIFieldDefinition_MIFieldTypeCode 
				FROM [Case] c INNER JOIN
				ClientMIDefinition cm ON c.Case_WorkTypeCode = cm.ClientMIDefinition_MIDEFCODE and cm.ClientMIDefinition_Inactive = 0 INNER JOIN
				ClientMIFieldSet cf ON cm.ClientMIDefinition_MIDEFCODE = cf.ClientMIFieldSet_ClientMIDEFCODE and cf.ClientMIFieldSet_Inactive = 0 INNER JOIN
				MIFieldDefinition mf ON cf.ClientMIFieldSet_MIFieldDefCode = mf.MIFieldDefinition_MIFieldCode and mf.MIFieldDefinition_Inactive = 0
				WHERE c.Case_CaseID = @pCase_CaseID and mf.MIFieldDefinition_DataField <> 'n/a' and ISNULL(mf.MIFieldDefinition_DataField,'') <> ''
			END
	...
	--FI WE ARE RETRIVING A COSTS SET
	IF @pGetCosts = 1
	BEGIN
		--WORKTYPE
