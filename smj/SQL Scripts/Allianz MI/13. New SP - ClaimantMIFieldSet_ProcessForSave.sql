...
/****** Object:  StoredProcedure [dbo].[ClaimantMIFieldSet_ProcessForSave]    Script Date: 05/08/2012 16:16:30 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	DECLARE @sUpdateSQL NVARCHAR(MAX)= ''
		...
		IF EXISTS (SELECT TOP 1 * FROM MIClaimantDetails where MIClaimantDetails_CaseID = @CaseID and MIClaimantDetails_ContactID = @ContactID)
		BEGIN
			SELECT @RecExists = 1
		END
					
		--DOES THE MATTER HAVE A CLIENT GROUP DATASET
		IF EXISTS(SELECT * FROM ClientMIDefinition WHERE ClientMIDefinition_CLIENTGROUPCODE = (SELECT Case_GroupCode FROM [CASE] WHERE Case_CaseID = 1987))
		BEGIN
			SELECT @MIDEFCODE =  cm.ClientMIDefinition_MIDEFCODE 
			FROM [Case] c INNER JOIN
			ClientMIDefinition cm ON c.Case_GroupCode = cm.ClientMIDefinition_CLIENTGROUPCODE and cm.ClientMIDefinition_Inactive = 0
		...
		FROM @XmlTable x
		INNER JOIN ClientMIFieldSet cm ON x.FieldCode = cm.ClientMIFieldSet_MIFieldDefCode and x.ClientMIDefinition_MIDEFCODE = cm.ClientMIFieldSet_ClientMIDEFCODE and cm.ClientMIFieldSet_Inactive = 0
		INNER JOIN MIFieldDefinition MI on cm.ClientMIFieldSet_MIFieldDefCode = MI.MIFieldDefinition_MIFieldCode
		...
		--PRINT @fieldtype + ' - ' + @PMIFieldValue + ' - '  + @DataField
